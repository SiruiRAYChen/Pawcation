...
export const RestaurantDetail = ({ place_id: propPlaceId }: RestaurantDetailProps) => {
  ...
  const [restaurantData, setRestaurantData] = useState<RestaurantDetailData | null>(null);
  ...
  const fetchRestaurantDetails = async () => {
    try {
      ...
      if (initialData?.nationalPhoneNumber) {
        setRestaurantData({
          id: placeId,
          name: initialData.name || "Unknown Restaurant",
      ...
      setRestaurantData({
        id: data.id || placeId,
        name: data.displayName?.text || initialData?.name || "Unknown Restaurant",
  ...
  const handleCall = () => {
    if (restaurantData?.nationalPhoneNumber) {
      window.location.href = `tel:${restaurantData.nationalPhoneNumber}`;
    } else {
      toast({
        title: "Phone number not available",
        description: "Contact information is not available for this restaurant",
        variant: "destructive",
      });
    }
  };
  ...
  return (
    <div className="min-h-screen bg-background pb-6">
      ...
      <div className="w-full h-64 bg-muted">
        {restaurantData.photos.length > 0 ? (
          <img
      ...
      >
        ...
        {/* Tab Content */}
        {activeTab === 'photos' && restaurantData.photos.length > 1 && (
      ...
      <div className="px-4 mt-6">
        <Button
          onClick={handleCall}
          disabled={!restaurantData.nationalPhoneNumber}
          className="w-full h-12 text-lg font-semibold"
        >
          {restaurantData.nationalPhoneNumber ? "Call Restaurant" : "Phone not available"}
        </Button>
      </div>

      {/* Website Link */}
      {restaurantData.websiteUri && (
        <div className="px-4 mt-3">
          <Button
            variant="outline"
            onClick={() => window.open(restaurantData.websiteUri, '_blank')}
            className="w-full h-12 text-lg font-semibold"
          >
            Visit Website
          </Button>
