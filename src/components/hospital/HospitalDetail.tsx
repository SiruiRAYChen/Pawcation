...
export const HospitalDetail = ({ place_id: propPlaceId }: HospitalDetailProps) => {
  ...
  const [hospitalData, setHospitalData] = useState<HospitalDetailData | null>(null);
  ...
  const initialData = location.state?.hospitalData;
  ...
  const fetchHospitalDetails = async () => {
    try {
      ...
      if (initialData?.nationalPhoneNumber) {
        setHospitalData({
          id: placeId,
          name: initialData.name || "Unknown Hospital",
      ...
      setHospitalData({
        id: data.id || placeId,
        name: data.displayName?.text || initialData?.name || "Unknown Hospital",
      ...
      if (initialData) {
        ...
        toast({
          title: "Failed to load hospital details",
  ...
  const handleCall = () => {
    if (hospitalData?.nationalPhoneNumber) {
      window.location.href = `tel:${hospitalData.nationalPhoneNumber}`;
    } else {
      toast({
        title: "Phone number not available",
        description: "Contact information is not available for this hospital",
        variant: "destructive",
      });
    }
  };
      ...
      <div className="min-h-screen bg-background">
        ...
        <div className="sticky top-0 z-10 bg-background border-b border-border px-4 py-3 safe-top">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-xl font-bold">Hospital Details</h1>
          </div>
        </div>
  ...
  if (error || !hospitalData) {
    return (
      <div className="min-h-screen bg-background">
        ...
        {/* Error State */}
        <div className="flex flex-col items-center justify-center py-16">
          <p className="text-destructive mb-4">{error || "Hospital not found"}</p>
  ...
  return (
    <div className="min-h-screen bg-background pb-6">
      ...
      >
          ...
          {hospitalData.rating && (
            <div className="flex items-center gap-2">
              <div className="flex items-center gap-1">
                ...
                <span className="font-medium">{hospitalData.rating.toFixed(1)}</span>
              </div>
        ...
        {/* Tab Content */}
        {activeTab === 'photos' && hospitalData.photos.length > 1 && (
            ...
            <div className="grid grid-cols-2 gap-2">
              {hospitalData.photos.slice(1, 5).map((photo, index) => (
                <div key={index} className="aspect-square bg-muted rounded-lg overflow-hidden">
                  <img
                    src={photo}
                    alt={`${hospitalData.name} photo ${index + 2}`}
                    className="w-full h-full object-cover"
                  />
                </div>
              ))}
      ...
      <div className="px-4 mt-6">
        <Button
          onClick={handleCall}
          disabled={!hospitalData.nationalPhoneNumber}
          className="w-full h-12 text-lg font-semibold"
        >
          {hospitalData.nationalPhoneNumber ? "Call Hospital" : "Phone not available"}
        </Button>
      </div>

      {/* Website Link */}
      {hospitalData.websiteUri && (
        <div className="px-4 mt-3">
          <Button
            variant="outline"
            onClick={() => window.open(hospitalData.websiteUri, '_blank')}
            className="w-full h-12 text-lg font-semibold"
          >
            Visit Website
          </Button>
